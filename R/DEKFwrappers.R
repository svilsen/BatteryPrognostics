...
#' @export
SODEFKFilter <- function(I, V, theta_0, ocv_0, SOC_0, C_max, eta,
...
#' @export
SODEFKSmooth <- function(I, V, theta_0, ocv_0, SOC_0, C_max, eta,
                         P = NULL, Q = NULL, R = NULL,
                         dt = 1, K = 2, trace = TRUE, traceLimit = 1e6) {
    if (is.null(P)) {
        P <- list(diag(10, K + 1), diag(10, length(theta_0)))
    ...
    res <- SODEFKSmoothCpp(I, V, theta_0, ocv_0, SOC_0, C_max, P, Q, R, dt, K, trace, traceLimit)
    return(res)
}
