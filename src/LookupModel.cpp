...
private :
    arma::mat Cap, R0, OCV;
    arma::colvec I, IF, IC, SOCList, IList;
...
public : 
    ...
    void SimulateModel() 
    {
        ...
        for (unsigned int n = 0; n < N / dt - 1; n++) 
        {
            ...
            {
                double w_n = 0.0;
                if (SOC_hat[n + 1] > SOCList[0])
                {
                    w_n = (SOC_hat[n + 1] - SOCList[j]) / (SOCList[j + 1] - SOCList[j]);
                }
                
                OCV_n = (1.0 - w_n) * OCV(j, i_f_) + w_n * OCV(j + 1, i_f_);
                R0_n = (1.0 - w_n) * R0(j, i_c) + w_n * R0(j + 1, i_f);
                
                for (std::size_t k = 0; k < K; k++)
                {
                    const double & Rk_n = (1.0 - w_n) * Rk[k](j, i_c) + w_n * Rk[k](j + 1, i_c);
                    const double & Ck_n = (1.0 - w_n) * Ck[k](j, i_c) + w_n * Ck[k](j + 1, i_c);
                    Vk_n[k] = VK(T_hat[n + 1], Vk_n[k], dt, I_n, Rk_n, Ck_n);
                }
            }
