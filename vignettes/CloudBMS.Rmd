...
# aging_week_1 <- vector("list", dim(aging_week_1_1)[1])
...
corrected_time <- correct_time(aging_week_1_1$Current, aging_week_1_1$Voltage, aging_week_1_1$Time)
aging_week_1 <- tibble("I" = corrected_time[, 1], "V" = corrected_time[, 2], "Time" = corrected_time[, 3], 
                       "TimeHour" = Time / 3600, "Day" = floor(TimeHour / 24)) %>% 
    filter(Time > 1500)

# Age 2
aging_is_week_2 <- which(stringr::str_detect(aging_files, "week2"))

aging_week_2_ <- vector("list", length(aging_is_week_2))
for (i in seq_along(aging_is_week_2)) {
    aging_week_2_i <- readr::read_delim(file = aging_files[aging_is_week_2[i]], 
                                        delim = "\t", skip = 3)
    aging_week_2_[[i]] <- aging_week_2_i %>% 
        select(Time = `Test Time`, Capacity, Current, Voltage, MD) %>% 
        mutate(Day = as.numeric(str_sub(Time, start = 3, end = 3)) + (i - 1), 
               Time = as.POSIXct(str_sub(Time, start = 6), format = "%H:%M:%OS"), 
               Time = as.numeric(Time - Time[1]) + Day * 24 * 3600, 
...
aging_week_2 <- bind_rows(aging_week_2_)
corrected_time <- correct_time(aging_week_2$Current, aging_week_2$Voltage, aging_week_2$Time)
aging_week_2 <- tibble("I" = corrected_time[, 1], "V" = corrected_time[, 2], "Time" = corrected_time[, 3],
                       "TimeHour" = Time / 3600, "Day" = floor(TimeHour / 24))
```
...
```{r , cache = TRUE}
current <- aging_week_1$I
voltage <- aging_week_1$V
...
MeanAPE(voltage, V_RCK$V_hat)
MaxAPE(voltage, V_RCK$V_hat)
summary(V_RCK$SOC)
```
...
vsb <- microbenchmark(
    V_simulated_1 <- SODEFKFilter(I = current, V = voltage, theta_0 = theta_0, 
...
summary(vsb)$median
MeanAPE(voltage, V_simulated_1$V_hat)
MaxAPE(voltage, V_simulated_1$V_hat)
...
ggplot(parameter_week_1 %>% filter(Time > 2500), aes(x = TimeDay / 3600, y = exp(R0))) + 
...
ggplot(parameter_week_1 %>% filter(Time > 2500), aes(x = TimeDay / 3600, y = Eta)) + 
    geom_line() + facet_wrap(~(Day + 1), nrow = 1) + theme_bw(base_size = 25) +
    xlab("Time of day") + ylab(bquote(eta)) +  
    scale_x_continuous(breaks = c(1, 5, 10, 15, 20, 24))

ggplot(parameter_week_1 %>% filter(Time > 2500, Alpha > -0.1), aes(x = TimeDay / 3600, y = Alpha)) + 
    geom_line() + facet_wrap(~(Day + 1), nrow = 1) + theme_bw(base_size = 25) +
...
par_initial <- c(0.0, 2.5)
lb <- c(0, 2) # -Inf, -Inf, 0, -Inf, 0, 0, -Inf, -Inf, -Inf, -Inf, -Inf, 
ub <- c(1.0, 3) # Inf, Inf, Inf, Inf, Inf, 1.0, Inf, Inf, Inf, Inf, Inf, 

DEFK_optim <- function(par, current, voltage, dt, K, ocv_0, theta_0) {
    # theta_ = par[1:length(theta_0)]
    # ocv_ = par[(length(theta_0) + 1):(length(theta_0) + length(ocv_0))]
    SOC_ = par[1] # (length(theta_0) + length(ocv_0) + 1)]
    C_max_ = par[2] # (length(theta_0) + length(ocv_0) + 2)]
    
    V_simulated <- SODEFKFilter(I = current, V = voltage, theta_0 = theta_0, ocv_0 = ocv_0, 
                                SOC_0 = SOC_, C_max = C_max_, eta = 1, dt = dt, K = K, 
                                trace = FALSE, traceLimit = 1)
    
    res = max(abs((V_simulated$V - V_simulated$V_hat) / V_simulated$V))
...
SOC_1 <- opt_par$par[1] # (length(theta_0) + length(ocv_0) + 1)]
C_max_1 <- opt_par$par[2] # (length(theta_0) + length(ocv_0) + 2)]

V_simulated <- SODEFKFilter(I = current, V = voltage, theta_0 = theta_0, ocv_0 = ocv_0,
                            SOC_0 = SOC_1, C_max = C_max_1, eta = 1, dt = dt, K = K,
                            trace = FALSE, traceLimit = 1)
...
```{r ageingtestweek2, cache = T}
current = aging_week_2$I
voltage = aging_week_2$V
...
ocv_0 <- c(3.11602927, 0.20411943, -0.12431090, 0.02695484, 0.0)
theta_0 <- c(V_simulated_1$Theta) # log(0.00523), log(0.00490), 2.527, log(0.0117), 36.748, 1.0, 0.0)

vsb <- microbenchmark(
    V_simulated_2 <- SODEFKFilter(I = current, V = voltage, theta_0 = theta_0,
                                  ocv_0 = ocv_0, SOC_0 = SOC_0,
                                  C_max = C_max, dt = dt, K = K,
                                  trace = trace, traceLimit = traceLimit), 
    times = 1
...
compare_tibble <- enframe(V_simulated_2[1:2], name = "Type", value = "Voltage") %>% 
    unnest() %>% 
    mutate(Time = rep(1:(n() / 2), times = 2), 
           Type = ifelse(Type == "V", "Measured voltage", "Simulated voltage"))
...
MeanAPE(voltage, V_simulated_2$V_hat)
MaxAPE(voltage, V_simulated_2$V_hat)
summary(V_simulated_2$SOC)
```

```{r}
parameter_week_2 <- aging_week_2 %>% 
    mutate(R0 = V_simulated_2$ThetaTrace[1, ], 
           R1 = V_simulated_2$ThetaTrace[2, ], 
           Tau1 = V_simulated_2$ThetaTrace[3, ], 
           R2 = V_simulated_2$ThetaTrace[4, ], 
           Tau2 = V_simulated_2$ThetaTrace[5, ], 
           Eta = V_simulated_2$ThetaTrace[6, ], 
           Alpha = V_simulated_2$ThetaTrace[7, ], 
           SOC = V_simulated_2$SOC, 
           SOCRound = mround(SOC, 5)) %>% 
    group_by(Day) %>% 
    mutate(TimeDay = 1:n())

pdf("~/AAU/PostDoc/Battery/BatteryPrognosticsR/TheveninModelR/Figures/DynamicAgedCellWeek2.pdf", width = 25, height = 7)
ggplot(parameter_week_2 %>% filter(Time > 2500), aes(x = TimeDay / 3600, y = exp(R0))) + 
    geom_line() + facet_wrap(~(Day + 1), nrow = 1) + theme_bw(base_size = 25) +
    xlab("Hour of day") + ylab(bquote(R[0])) + 
    scale_x_continuous(breaks = c(1, 5, 10, 15, 20, 24))

ggplot(parameter_week_2 %>% filter(Time > 2500), aes(x = TimeDay / 3600, y = Eta)) + 
    geom_line() + facet_wrap(~(Day + 1), nrow = 1) + theme_bw(base_size = 25) +
    xlab("Time of day") + ylab(bquote(eta)) + 
    scale_x_continuous(breaks = c(1, 5, 10, 15, 20, 24))

ggplot(parameter_week_2 %>% filter(Time > 2500, Alpha > -0.1), aes(x = TimeDay / 3600, y = Alpha)) + 
    geom_line() + facet_wrap(~(Day + 1), nrow = 1) + theme_bw(base_size = 25) +
    xlab("Time of day") + ylab(bquote(alpha)) + 
    scale_x_continuous(breaks = c(1, 5, 10, 15, 20, 24))
dev.off()

pdf("~/AAU/PostDoc/Battery/BatteryPrognosticsR/TheveninModelR/Figures/DynamicAgedCellWeek2_2.pdf", width = 25, height = 7)
ggplot(parameter_week_2 %>% filter(Time > 2500), aes(x = SOC, y = exp(R0)), colour = as.factor(Day + 1)) + 
    geom_line() + #facet_wrap(~(Day + 1), nrow = 1) + 
